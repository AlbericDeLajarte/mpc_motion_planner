...
int main(int, char**) { 
    ...
    for(int iter =0; iter<1000; iter++){   
        ...
        for (int iPoint = 0; iPoint<=nPoints; iPoint++)
        {
            ...
            pinocchio::updateFramePlacement(planner.robot.model, planner.robot.data, planner.robot.frame_id);
            if(planner.robot.data.oMf[planner.robot.frame_id].translation()[2] < 0) {
                collision_flag_rk = 0;
                std::cout << "RK Table collision: " << planner.robot.data.oMf[planner.robot.frame_id].translation()[2] << std::endl;
            }
        }
        ...
        {            
            ...
            if(planner.robot.data.oMf[planner.robot.frame_id].translation()[2] < 0) {
                ...
                std::cout << "MPC Table collision: " << planner.robot.data.oMf[planner.robot.frame_id].translation()[2] << std::endl;
            }
        }
